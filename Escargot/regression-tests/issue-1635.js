...
// outer class's same-named brand
class L {
  #v = 13;
  make = () => {
    class Inner {
...
// must reach the enclosing class's name through the outer-class chain
class M {
  #x = 'outer';
  m() {
    class Mid {
...
assert(throughObjectMethod.read(o) === 'ox');
assert(throughObjectMethod.write(o, 'updated') === 'updated');
assert(throughObjectMethod.read(o) === 'updated');

// a name shadowed by an intermediate class must resolve to that class and
// must not be served by the outer class's member on an instance that
// carries the inner and outer brands but not the intermediate one
class P {
  #x = 'outer';
  value() { return this.#x; }
  m() {
...
var pOuter = new P();
var pPair = pOuter.m();
assert(pPair.leaf.read(pPair.mid) === 'mid');
assertThrows(function() { pPair.leaf.read(pPair.leaf); }, TypeError);
assertThrows(function() { pPair.leaf.write(pPair.leaf); }, TypeError);
assert(pPair.leaf.value() === 'outer');
assert(pOuter.value() === 'outer');
